...
class UnsqueezeTest: AnnotationSpec() {
    @Test fun forward() {
        val x = FloatTensor(Shape(2)) { it.toFloat() }
        fun f(x: DTensor): DTensor = x.unsqueeze(1)
        val der0 = f(x)
        val der1 = forwardDerivative1(x, ::f)
        val der2 = forwardDerivative2(x, ::f)
        der0 shouldBeExactly FloatTensor(Shape(2, 1), floatArrayOf(0f, 1f))
        der1 shouldBeExactly FloatTensor(Shape(2, 1, 2), floatArrayOf(1f, 0f, 0f, 1f))
        der2 shouldBeExactly FloatTensor(Shape(2, 1, 2, 2)) { 0f }
    }
    ...
    @Test fun reverse() {
        ...
        der2 shouldBeExactly FloatTensor(Shape(2, 2, 2, 1)) { 0f }
    }
}